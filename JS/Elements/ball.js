...
function collision_rev(){
    ...
    for(var i=0;i<listCalcLines.length;i++){
        ...
        else{ //in case of vertical element
            p = startX;
            p1 = lastCoord[0];
            p2 = coord[0];
        }
        if(Math.sign(p - p1) !== Math.sign(p - p2) || Math.sign(p-p2) <= ballRadius){ //https://fr.wikipedia.org/wiki/Demi-plan
            var x,y;
            //calculate equation of ball trajectory
            if(lastCoord[0] !== coord[0]) { // not vertical rebound
            ...
            if(x > startX && x < endX || y > startY && y < endY) {
                // dp = Math.abs(p-p1);
                // if(Math.abs(p-p1) > dp) { //if not the next collision
                //     continue;
                // }
